...
<Callout type="info" title="Parameter Detection">
  ...
  signature. It only looks at the parameter definitions—it doesn't execute your
...
- `description` - Explicit tool description (if not provided, falls back to JSDoc)
- `isLongRunning` - Mark as long-running operation
- `shouldRetryOnFailure` - Enable automatic retries
...
- **agent** (`BaseAgent`): The agent instance to use as a tool (required)
- **description** (`string`, optional): Description of what the tool does (defaults to agent's description)
- **skipSummarization** (`boolean`, optional): If `true`, returns the agent's response directly without additional LLM processing. If `false` (default), the main agent's LLM processes the response before including it in output
