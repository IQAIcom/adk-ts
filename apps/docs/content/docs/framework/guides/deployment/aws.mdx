...
Your agent needs these environment variables configured:
...
- `LLM_TEMPERATURE` - Optional: Temperature setting (0.0-1.0)
- `LLM_MAX_TOKENS` - Optional: Maximum tokens for responses

**Agent-Specific Variables:**

- `DISCORD_TOKEN`, `DISCORD_CLIENT_ID` - For Discord bots
- `TELEGRAM_BOT_TOKEN` - For Telegram bots
- `SLACK_BOT_TOKEN`, `SLACK_SIGNING_SECRET` - For Slack bots
- `DATABASE_URL` - For database connections

<Callout type="warn" title="Never Commit Secrets">
  Never hardcode API keys in your code. Always use environment variables and AWS
...
<Tab value="ECS Fargate">
  ...
  Reference in task definition with `secrets` field - Never use plain
  `environment` field for sensitive data
</Tab>
