...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tab value="npm">```bash npm install -g @iqai/adk-cli adk new ```</Tab>
  <Tab value="yarn">```bash yarn global add @iqai/adk-cli adk new ```</Tab>
  <Tab value="pnpm">```bash pnpm add -g @iqai/adk-cli adk new ```</Tab>
</Tabs>
...
<Tabs items={["npm", "yarn", "pnpm"]}>
  ...
  <Tab value="yarn">```bash yarn add @iqai/adk ```</Tab>
...
If you get authentication errors, ensure: - You have imported and configured
dotenv in your agent file:
...
<Cards>
  ...
  <Card
    title="🤖 Agents"
    description="Explore different agent types and how to create your own."
  ...
  <Card
    title="📚 Tutorials"
    description="Learn through 16 hands-on examples covering agents, tools, memory, and advanced features"
