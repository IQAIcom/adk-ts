...
Events are the fundamental building blocks of communication in the Agent Development Kit (ADK). Every interaction, from a user's initial message to the agent's final response, flows through a series of structured events. Understanding events is essential for building, debugging, and orchestrating sophisticated agent systems.

## What Are Events?

An `Event` in ADK represents a single, immutable moment in your agent's execution timeline. Think of events as snapshots that capture everything happening at a specific point: user messages, agent responses, tool calls, state changes, and control signals.
...
<Callout type="info">
The complete sequence of events is stored in `session.events`, providing a chronological audit trail invaluable for debugging and understanding agent behavior.
</Callout>
...
```
<Callout title="Clarification" icon="💡"> `turnComplete` is a **conceptual field**, not part of the concrete `EventOpts` type. It’s used as a logical marker in documentation and streaming examples to indicate when an agent has finished a turn of reasoning or output. While not emitted as a typed field by default, some runtime visualizers or logs may simulate it for readability. </Callout>
...
<Callout type="warn">
Always check for the presence of content and parts before accessing nested properties. TypeScript's optional chaining (`?.`) is your friend here.
</Callout>
...
const agentEvent: Event = {
  ...
  timestamp: Date.now() / 1000,
  content: {
    parts: [{ text: 'I can help you find flights. What date would you like to travel?' }]
  },
  partial: false,
  turnComplete: true,
  actions: {}
...
interface EventActions {
  ...
  // Artifact tracking
  artifactDelta?: Record<string, Part>;  // Saved artifacts with version info
...
<Callout>
State keys can use prefixes to indicate scope:
- `app:` - Application-wide settings
- `user:` - User-specific preferences (across sessions)
- `temp:` - Temporary session-only data
- No prefix - Default session scope
</Callout>
...
// This generates an event with artifact delta
const event: Event = {
  author: 'ReportAgent',
  actions: {
    artifactDelta: {
...
<Callout type="info">
The `SessionService` is the gatekeeper for state persistence. It ensures that every state change and artifact update is properly recorded and tied to the event history.
</Callout>
...
for await (const event of runner.runAsync(query, session)) {
  ...
  if (event.isFinalResponse()) {
    ...
    if (event.actions?.skipSummarization && event.getFunctionResponses().length > 0) {
...
```typescript
const statefulAgent = async (agent: LlmAgent, query: string, session: Session) => {
...
const debugSession = async (agent: LlmAgent, query: string, session: Session) => {
  ...
  session.events.forEach((event, index) => {
    ...
    if (event.getFunctionCalls().length > 0) {
      console.log(`  Tool Calls:`, event.getFunctionCalls().map(c => c.name));
    }
