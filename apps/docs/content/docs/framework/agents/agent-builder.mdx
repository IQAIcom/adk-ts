...
const response = AgentBuilder.withModel("gemini-2.5-flash").ask(
...
async function main() {
  const { runner } = await AgentBuilder.create("my-assistant")
...
const { runner } = AgentBuilder.create("my-agent")
  .withModel("gemini-2.5-flash")
  .build();
```
...
const response = AgentBuilder.withModel("gemini-2.5-flash").ask("Hello!");
```
...
const agent = AgentBuilder.create("my-assistant")
  .withModel("gemini-2.5-flash")
...
const builder =
  AgentBuilder.create("research_assistant").withModel("gemini-2.5-flash");
const builder2 =
  AgentBuilder.create("data_processor").withModel("gemini-2.5-flash");
```
...
const agent1 = AgentBuilder.create("my_agent")
  .withModel("gemini-2.5-flash")
...
const agent2 = AgentBuilder.create("my_agent")
  .withModel("gemini-2.5-flash")
...
const agent = AgentBuilder.create("financial_analyst")
  .withModel("gemini-2.5-flash")
  .withDescription(
...
// Static instruction
const financeAgent = AgentBuilder.create("financial_analyst")
  .withModel("gemini-2.5-flash")
  .withInstruction(
...
const travelAgent = AgentBuilder.create("travel_agent")
  .withModel("gemini-2.5-flash")
  .withInstruction(
...
// User asks the agent to analyze the provided document
const response = await agent.runner.ask("Analyze this document...");

console.log(response); // The output will match the defined schema
```
...
const agent = await AgentBuilder.create("monitored_agent")
  ...
  .withBeforeModelCallback(async (context) => {
    console.log("Before model execution:", context);
    return undefined;
  })
  .build();
```
...
const agent = await AgentBuilder.create("monitored_agent")
  .withModel("gemini-2.5-flash")
  .withDescription("An agent with monitoring capabilities")
  .withAfterModelCallback(async (context) => {
    console.log("After model execution:", context);
    return undefined;
  })
  .build();
```
...
const response = await agent.runner.ask("Can you summarize where we left off?");
```
