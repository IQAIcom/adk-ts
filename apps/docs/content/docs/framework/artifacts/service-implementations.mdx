...
The `deleteArtifact` method is available on all artifact service
implementations but is intentionally not exposed through context objects
(`CallbackContext` or `ToolContext`) for safety reasons. Access it directly
through the artifact service instance. See [Best
...
Data stored in `InMemoryArtifactService` is lost when the process restarts.
...
The GCS service automatically handles retries for transient errors and
provides built-in redundancy and durability.
