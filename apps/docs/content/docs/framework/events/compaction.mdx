...
Event Compaction uses a sliding window algorithm that triggers after a
configured number of new invocations, ensuring consistent behavior and
predictable costs.
...
<Step>
  ### Trigger Condition After each agent turn completes, ADK checks if enough
  new invocations have occurred since the last compaction.
</Step>

<Step>
  ### Window Selection When triggered, ADK selects a window of invocations to
  compact, including some overlap from the previous compaction for continuity.
</Step>

<Step>
  ### Summarization The selected events are sent to an LLM (by default, the
  agent's own model) to generate a concise summary.
</Step>

<Step>
  ### Storage The summary is stored as a special compaction event with
  `actions.compaction` containing the timestamp range and summarized content.
</Step>

<Step>
  ### Consumption When building LLM requests, compaction events are converted to
  normal model events and original covered events are filtered out.
</Step>
...
Choose your `compactionInterval` based on your use case: - **Higher values
(20+)**: Less frequent compaction, more detailed history retained - **Lower
values (5-10)**: More aggressive compaction, better for very long sessions
  ...
  return {
    ...
    totalEventsCompacted: compactions.reduce((sum, e) => {
      ...
      const covered = session.events.filter(
        ev =>
          ev.timestamp >= compact.startTimestamp &&
...
• Key point 3`;
    ...
    for await (const response of this.model.generateContentAsync({
      ...
      summary += response.content?.parts?.map(p => p.text || "").join("");
    }
...
If compaction isn't occurring, check: 1. `eventsCompactionConfig` is set on
the Runner 2. `compactionInterval` threshold has been reached 3. Summarizer is
properly configured 4. Session has enough invocations
...
class SelectiveSummarizer implements EventsSummarizer {
  async maybeSummarizeEvents(events: Event[]): Promise<Event | undefined> {
    ...
    const textEvents = events.filter(
      e => e.content?.parts?.some(p => p.text) && !e.actions?.stateDelta, // Preserve state changes
    );
...
- [Session](/docs/framework/session-state-memory/session) - Managing conversation state
- [Memory](/docs/framework/session-state-memory/memory) - Long-term storage patterns
