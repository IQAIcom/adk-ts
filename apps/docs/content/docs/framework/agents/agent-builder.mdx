...
| `withFallbackModels(...models)` | `string[]`                           | Configures fallback models for rate limit recovery |
...
```typescript
const builder =
  await AgentBuilder.create("research_assistant").withModel("gemini-2.5-flash");
const builder2 =
  await AgentBuilder.create("_data_processor2").withModel("gemini-2.5-flash");
...
Configures fallback models to use when the primary model encounters rate limit errors (HTTP 429). When rate limited, the system first retries the current model up to 3 times with a 1-second delay between attempts. If retries are exhausted, it automatically switches to the next fallback model in the provided order for the current request only.

This provides resilience against rate limits without requiring manual intervention, making your agents more reliable in production environments with varying API quotas.
...
For advanced configuration (custom retry counts, delays), use the `ModelFallbackPlugin` directly. See [Model Fallback Plugin](/docs/framework/plugins/model-fallback-plugin) for details.
