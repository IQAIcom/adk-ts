...
| `model`                    | ❌\*     | `string \| BaseLlm \| LanguageModel` | LLM model to use (\*inherits from parent if not set) |
| `instruction`              | ❌       | `string \| InstructionProvider`      | Primary behavior instructions                        |
| `globalInstruction`        | ❌       | `string \| InstructionProvider`      | Global instructions for entire agent tree            |
| `tools`                    | ❌       | `ToolUnion[]`                        | Available tools for the agent                        |
...
| `includeContents`          | ❌       | `"default" \| "none"`                | Context inclusion behavior                           |
...
const agent = new LlmAgent({
  name: "customer_support_agent", // ✅ Valid
