...
<Callout type="info" title="Production Deployment">
  ...
  See the [Production Configuration](/docs/framework/observability/production)
...
<Callout type="info">
  The ADK telemetry service automatically appends `/v1/traces` and `/v1/metrics`
  to the OTLP endpoint, so you should only specify the base URL without the path
  suffix.
</Callout>
