...
<Callout type="info" title="Division of Responsibilities">
  Your agent handles the "thinking"—understanding user intent, deciding which
  tools to use, and interpreting results. Your tools handle the
  "doing"—executing specific operations and returning structured data for the
  agent to work with.
</Callout>
...
sequenceDiagram
    ...
    T->>A: {status: "success", temperature: "18°C", conditions: "cloudy"}
    A->>A: Process tool output
    A->>U: "The weather in London is currently 18°C and cloudy"
