...
Your agent inherits the model from its parent if you omit this. Your model choice also affects its quality, speed, and cost, so choose wisely. See [Models & Providers](/docs/framework/agents/models) for details.
