...
<Callout type="info" title="Why 'worker' not 'web'?">
  The `web` process type expects your app to bind to `$PORT`. Since Telegram
  bots don't serve HTTP, use `worker` to run as a background process.
</Callout>
...
- [MCP Telegram Documentation](/docs/mcp-servers/telegram) — Learn about MCP Telegram features and configuration
- [Docker Deployment Guide](/docs/framework/guides/deployment/docker) — Advanced Docker deployment options
- [Railway Deployment Guide](/docs/framework/guides/deployment/railway) — Detailed Railway setup
