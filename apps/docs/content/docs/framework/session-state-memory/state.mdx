...
async function runAgent() {
  const { runner } = await AgentBuilder.create("guide")
    .withModel("gemini-2.0-flash")
    .withInstruction("Welcome! Theme: {user:theme?}. Step: {current_step}.")
    .withSessionService(sessionService, {
      userId: "user123",
      appName: "my-app",
      initialState: { current_step: "welcome", "user:theme": "dark" },
    })
    .build();

  const result = await runner.query({ message: "Start" });
  return result;
}
```
...
const event = new Event({
  actions: new EventActions({ stateDelta: { "user:login_count": 1 } }),
});
...
- **Clear Prefixes**: Use `user:`, `app:`, or `temp:` prefixes with descriptive key names for better organization
