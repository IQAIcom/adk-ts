...
Persistent storage using SQLite, PostgreSQL, or MySQL. Quick-start with the factory helpers:

```typescript
import { createDatabaseSessionService, createSqliteSessionService } from '@iqai/adk';

// Easiest: SQLite (install peer dep: better-sqlite3)
const sessionService = createSqliteSessionService('sessions.db');

// Or auto-detect from URL (install appropriate peer dep: pg/mysql2/better-sqlite3)
