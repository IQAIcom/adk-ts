...
- **Market Data**: CoinGecko, CoinGecko Pro, DefiLlama, DeBank, Upbit
- **Prediction Markets**: Polymarket, Kalshi, Limitless, Opinion
- **Messaging & Social**: Discord, Telegram
- **Information**: IQWiki

Browse the sidebar to explore each MCP server's documentation, including setup instructions, configuration options, available tools, and usage examples.

## Third-Party Wrappers

ADK-TS also provides convenience wrappers for popular third-party MCP servers:

- **Filesystem** - File system operations using Anthropic's filesystem server
- **Memory** - Memory and note-taking capabilities using Anthropic's memory server

Browse the sidebar for detailed documentation on each third-party wrapper.
