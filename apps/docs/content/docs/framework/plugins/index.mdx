...
An ADK-TS Plugin extends the `BasePlugin` class and contains one or more callback methods, indicating where in the agent lifecycle the Plugin should be executed. You integrate Plugins into an agent by registering them on a `Runner`, via `AgentBuilder.withPlugins()`, or directly on an `LlmAgent`. For more information on how and where you can trigger Plugins in your agent application, see Plugin callback hooks.

Plugin functionality builds on [Callbacks](/docs/framework/callbacks), which is a key design element of the ADK-TS extensible architecture. While a typical Agent Callback is configured on a single agent, a single tool for a specific task, a Plugin is most commonly registered on the `Runner` for global scope, though agent-level registration is also supported.

Runner-level plugin callbacks apply globally to every agent, tool, and LLM call managed by that runner. Plugins let you package related callback functions together to be used across a workflow. This makes Plugins an ideal solution for implementing features that cut across your entire agent application.
...
<Tabs items={['Runner (Global)', 'AgentBuilder (Global)', 'LlmAgent (Agent-Scoped)']}>
  <Tab value="Runner (Global)">
    Register Plugins on the `Runner` for global scope — they apply to all Agents, Tools, and LLM calls managed by that runner. You can set a `pluginCloseTimeout` to bound shutdown time.
  ...
  <Tab value="AgentBuilder (Global)">
    The `AgentBuilder` provides a fluent API for plugin registration via `withPlugins()`. Plugins registered this way are passed through to the Runner that `build()` creates, so they also have global scope.
    ...
    const { runner } = await AgentBuilder.withModel("gemini-1.5-flash")
      ...
      .withPlugins(new BasicLoggingPlugin())
      .build();
    ```

  </Tab>

  <Tab value="LlmAgent (Agent-Scoped)">
    You can attach plugins directly to an `LlmAgent` via its constructor. Agent-level plugins are scoped to that specific agent rather than applying globally via the Runner.
...
| ---------------------------- | -------------------------------- | ----------------------------------------------- |
| `Runner` / `InMemoryRunner`  | Global (all agents, tools, LLMs) | Cross-cutting concerns like logging, monitoring |
| `AgentBuilder.withPlugins()` | Global (via built Runner)        | Fluent API preference                           |
| `LlmAgent({ plugins })`      | Agent-scoped                     | Plugin should only apply to a specific agent    |
...
```typescript
import { LlmAgent, InMemoryRunner, BaseTool } from "@iqai/adk";
import { BasicLoggingPlugin } from "./basic-logging-plugin";
...
<Callout type="info" title="Error Type">
  The `error` parameter is typed as `unknown` in `BasePlugin`. In practice,
  `PluginManager` always passes an `Error` instance, but defensive type guards
  (e.g., `params.error instanceof Error`) are recommended for forward
  compatibility.
</Callout>
...
These guidelines apply to all plugins — both prebuilt and custom. For plugin-specific guidance, see the individual plugin pages.
...
3. **Document Intervention Behavior**: Clearly document when your plugin will intervene (return non-undefined) to help users understand the impact.
...
1. **Start Conservative**: Begin with default configuration and adjust based on observations. All prebuilt plugins ship sensible defaults — tune only after measuring.
2. **Monitor and Test**: Track plugin behavior in development before production. Log callback frequency, latency, and intervention rate to catch issues early.
...
- **Timeout Protection**: Use the `pluginCloseTimeout` to prevent hanging during shutdown

### Multi-Plugin Usage

- **Order Matters**: Plugins execute in registration order. Place filtering plugins before retry plugins (e.g., `ToolOutputFilterPlugin` before `ReflectAndRetryToolPlugin`).
- **Intervention Hierarchy**: The first plugin to return a non-undefined value short-circuits the rest. Plan your registration order accordingly.
- **Resource Management**: Use proper cleanup in `close()` methods. The Runner calls close on all plugins during shutdown.
- **Unique Names**: Each plugin must have a unique name. Registration will throw if a duplicate name is detected.
...
| ------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Plugin not executing      | Not registered on Runner/AgentBuilder/LlmAgent | Verify registration; check that `build()` was called for AgentBuilder                                         |
| Callback not firing       | Wrong callback method name or signature        | Verify you implemented the exact method name from `BasePlugin` (e.g., `afterToolCallback`, not `onAfterTool`) |
| Plugin overriding another | Registration order conflict                    | Reorder plugins — first to return non-undefined wins                                                          |
| Shutdown hanging          | Plugin `close()` never resolves                | Set `pluginCloseTimeout` on the Runner (default: 5000ms)                                                      |
| Duplicate name error      | Two plugins share the same name                | Give each plugin a unique name in its constructor                                                             |

For plugin-specific troubleshooting, see the individual plugin pages. For general framework issues, see the [troubleshooting guide](/docs/framework/guides/troubleshooting).
...
```typescript
import { BasePlugin, BaseTool, ToolContext } from "@iqai/adk";
