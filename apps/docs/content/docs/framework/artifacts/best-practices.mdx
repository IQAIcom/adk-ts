...
## Artifact Cleanup Guidelines

Keep cleanup logic simple:

- Use the `deleteArtifact` method sparingly and only when you are sure the file is no longer needed.
- Always call it through your artifact service instance (it is not exposed via contexts).
- Prefer predictable filenames (for example, `temp_*`) so you can target specific artifacts if you need to delete them later.
