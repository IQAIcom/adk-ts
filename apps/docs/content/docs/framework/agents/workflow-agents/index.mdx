...
```typescript
import { AgentBuilder, LlmAgent, GoogleSearch } from "@iqai/adk";
...
const factCheckerAgent = new LlmAgent({
  ...
  tools: [new GoogleSearch()], // Can use tools for research
});

// Sequential workflow: write → edit → fact_check_agent
const { runner } = await AgentBuilder.create("content_pipeline_agent")
...
// Use it
const result = await runner.ask("Write about the benefits of renewable energy");
```
