...
const billing_specialist_agent = new LlmAgent({
  name: "billing_specialist_agent",
  description: "Handles billing inquiries and payment issues",
});

const content_validator_agent = new LlmAgent({
...
const vagueCoodinator = new LlmAgent({
  ...
  while (attempt < maxRetries) {
    try {
      ...
      yield new Event({
        ...
        content: {
          parts: [{
            text: `⚠️ Attempt ${attempt}/${maxRetries} failed: ${error.message}`,
          }],
        },
      });
