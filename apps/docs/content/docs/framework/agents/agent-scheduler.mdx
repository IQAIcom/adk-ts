...
The `AgentScheduler` runs ADK agents on a recurring schedule using cron expressions or fixed intervals. A single instance manages multiple jobs concurrently, each with its own schedule, callbacks, and execution tracking. It also handles overlap prevention, graceful shutdown, and real-time event streaming.
...
// 1. Build your agent
const { runner } = await AgentBuilder.create("daily-reporter")
...
scheduler.schedule({
  ...
  onEvent: (jobId, event) => {
    // Fires for every ADK event during execution
