...
- Use clear, descriptive filenames; extensions (e.g., `.pdf`, `.png`) help humans, while decoding relies on `mimeType`.
- Filenames must be unique within their scope (session or user namespace).
- Prefer consistent naming conventions for temporary vs long-lived artifacts (e.g., `tmp_*.json`).
