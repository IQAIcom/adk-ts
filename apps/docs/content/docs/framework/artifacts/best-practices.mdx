---
title: Best Practices for Artifact Management
...
For general ADK-TS best practices, see the [Best Practices
...
async function loadWithCache(
  ...
  const k = `${key.appName}:${key.userId}:${key.sessionId}:${key.filename}:${
    key.version ?? "latest"
  }`;
...
const ok =
  Buffer.from(part.inlineData.data, "base64").length <= 10 * 1024 * 1024;
```
...
function validatePart(filename: string, part: Part) {
  ...
  if (mime === "application/json") {
    try {
      JSON.parse(Buffer.from(part.inlineData.data, "base64").toString());
    } catch {
...
The `deleteArtifact` method is intentionally not exposed through context
objects (`CallbackContext` or `ToolContext`) for safety reasons. It must be
accessed directly through the artifact service instance.
...
class ArtifactMetricsCollector {
  ...
  getMetrics() {
    ...
    return {
      ...
      errorRate:
        (this.metrics.errors /
          (this.metrics.saves + this.metrics.loads + this.metrics.errors)) *
        100,
    };
  }
...
Always test artifact operations thoroughly in staging environments before
