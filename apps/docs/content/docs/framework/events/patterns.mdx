...
description: Common architectural patterns and best practices for event-driven ADK-TS applications
---
...
class EventStore {
  ...
  getEventHistory(sessionId: string): Event[] {
    return this.events.filter(
      e => e.invocationId === sessionId || e.content?.sessionId === sessionId,
    );
  }
...
class EventDeduplicator {
  ...
  constructor(private ttl = 300000) {
    // 5 minutes
...
These patterns provide building blocks for sophisticated event-driven
architectures. Choose patterns that fit your specific use case and complexity
requirements.
