...
Events are never actually deleted—they're kept in the history but filtered out
when building LLM context. This maintains audit trails while effectively
...
await app.sendMessage(
  "user-1",
  "session-1",
  "Actually, tell me about quantum physics",
);
...
Rewind only affects session state and artifacts managed by ADK. External side
effects (API calls, database writes, sent emails) cannot be automatically
undone.
