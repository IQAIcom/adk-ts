...
## Quick Connection with McpGeneric

For any npm-based MCP server, `McpGeneric` uses the same config pattern as the built-in wrappers, without the full `McpToolset` setup:

```typescript
import { McpGeneric, LlmAgent } from "@iqai/adk";

// Connect to any npm MCP package in one line
const toolset = McpGeneric("@modelcontextprotocol/server-brave-search", {
  env: {
...
`McpGeneric` handles `PATH` injection, env value coercion, and retry defaults automatically, the same as every other wrapper function. Use `McpToolset` directly when you need SSE transport, custom timeouts, or other options not exposed by the wrappers.
