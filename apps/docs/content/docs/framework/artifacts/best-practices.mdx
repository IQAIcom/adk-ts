...
class ArtifactCleanupService {
  ...
  async cleanupTemporaryArtifacts(
    ...
    for (const filename of artifacts) {
      // Match pattern if provided (e.g., 'temp_*', 'cache_*')
  ...
  async cleanupOldSessions(
    ...
    const cutoffTimestamp = Math.floor(
      (Date.now() - olderThanDays * 24 * 60 * 60 * 1000) / 1000
    );

    let deletedCount = 0;

    for (const sessionId of sessionIds) {
      // Get session to check its age
      ...
      // Only delete artifacts for sessions older than cutoff
      if (session.lastUpdateTime >= cutoffTimestamp) {
        console.log(
          `Session ${sessionId} is not old enough (last update: ${new Date(session.lastUpdateTime * 1000).toISOString()}), skipping`
        );
        continue;
      }
  ...
  private matchesPattern(filename: string, pattern: string): boolean {
    // Escape regex metacharacters, then convert * wildcards to .*
    // First escape all special regex characters to prevent accidental matches
