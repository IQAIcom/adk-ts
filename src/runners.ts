...
import { type BaseArtifactService, InMemoryArtifactService } from "./artifacts";
import {
	type BaseMemoryService,
	InMemoryMemoryService,
	InMemorySessionService,
} from "./memory";
import { InvocationContext, RunConfig, StreamingMode } from "./models";
import {
	type LiveRequest,
	LiveRequestQueue,
} from "./models/request/LiveRequestQueue";
...
export class Runner {
	...
	async *runAsync(options: {
		...
		if (runConfig.loadSession) {
			const existingSession = this.sessionService.getSession(
				this.appName,
				userId,
				sessionId,
			);
			if (existingSession) {
				...
				console.log(
					`Session not found. Creating a new session with ID: ${sessionId}`,
				);
	...
	protected _newInvocationContext(options: {
		...
		if (runConfig.supportCfc && "canonicalModel" in agent) {
			const modelName = (
				agent as unknown as { canonicalModel?: { modelName: string } }
			).canonicalModel?.modelName;
			if (modelName && !modelName.startsWith("gemini-2")) {
				throw new Error(
					`CFC is not supported for model: ${modelName} in agent: ${agent.name}`,
				);
			}
			...
			if (
				"tools" in agent &&
				Array.isArray((agent as unknown as { tools: BaseTool[] }).tools)
			) {
				// Check if the tool is already added
				const hasCodeExecution = (
					agent as unknown as { tools: BaseTool[] }
				).tools.some((tool: BaseTool) => tool.name === "code_execution");

				if (!hasCodeExecution) {
					(agent as unknown as { tools: BaseTool[] }).tools.push(
				...
				if (
					...
					Array.isArray(
						(agent as unknown as { canonicalTools: BaseTool[] }).canonicalTools,
					)
				) {
					const hasCanonicalCodeExecution = (
						agent as unknown as { canonicalTools: BaseTool[] }
					).canonicalTools.some(
					...
					if (!hasCanonicalCodeExecution) {
						(
							agent as unknown as { canonicalTools: BaseTool[] }
						).canonicalTools.push(builtInCodeExecution);
					}
				}
			}
		}
	...
	protected async _appendNewMessageToSession(
		...
		if (!context.session || !context.userContent) {
			throw new Error(
				"Session and user content must be provided to append a message",
			);
		}
	...
	protected _findAgentToRun(context: InvocationContext): BaseAgent {
		...
		const foundAgent = this._findAgentByName(
			this.agent,
	...
	protected _isTransferableAcrossAgentTree(agentToCheck: BaseAgent): boolean {
		...
		if (
			(agentToCheck as unknown as { disallowTransferToParent: boolean })
		...
		while (currentAgent) {
			...
			if (
				(currentAgent as unknown as { disallowTransferToParent: boolean })
					.disallowTransferToParent
			) {
				return false;
			}
			...
			const parent = (currentAgent as unknown as { parentAgent: BaseAgent })
				.parentAgent;
			// Explicitly cast the parent to the type required by the loop checks
			currentAgent = parent as BaseAgent &
				Record<"disallowTransferToParent", unknown>;
		}
	...
	async *runLive(options: {
		...
		const agentToRun = this._findAgentToRun(
			this._newInvocationContext({
				agent: this.agent,
				runConfig,
				session,
...
export class InMemoryRunner extends Runner {
	constructor(agent: LlmAgent, appName = "InMemoryRunner") {
		super({
			appName,
			agent,
