...
Event Actions are ADK's mechanism for signaling side effects, managing state, and controlling execution flow. While event `content` carries the communication payload (messages, tool calls), the `actions` property carries the instructions for _what should happen_ as a result of that event.
...
<Callout type="info">
  EventActions follow a declarative pattern: agents describe *what* should
  change, and the framework handles *how* and *when* to apply those changes.
</Callout>
...
| _(none)_ | Session     | Session     | Conversation context, current task state      |

<Callout>
  The actual persistence behavior depends on your `SessionService`
  implementation. The prefixes are conventions that help organize state and
  indicate intent.
</Callout>
...
<Callout type="info">
  The framework automatically collects state changes made during execution and
  includes them in the `stateDelta` of the next emitted event. You don't need to
  manually construct the `stateDelta` object.
</Callout>
...
const trackArtifacts = async (
  ...
  for await (const event of agent.run(query, { session })) {
    if (event.actions?.artifactDelta) {
      Object.entries(event.actions.artifactDelta).forEach(
        ([filename, part]) => {
          console.log(`📁 Artifact saved: ${filename}`);
          console.log(
            `   MIME type: ${part.inlineData?.mimeType || "unknown"}`
          );
          console.log(
            `   Size: ${part.inlineData?.data?.length || 0} bytes (base64)`
          );
...
<Callout>
  Agent transfers are processed by the `Runner`. When it encounters a
  `transferToAgent` action, it hands control to the specified agent while
  maintaining the same session and conversation history.
</Callout>
  ...
  async *runAsyncImpl(context: InvocationContext): AsyncGenerator<Event> {
    ...
    if (!isValid && retryCount < maxRetries) {
      ...
      yield new Event({
        ...
        content: {
          parts: [
            { text: "Validation failed. Please provide correct information." },
          ],
        },
      });
    } else if (!isValid) {
    ...
    try {
      ...
      if (errorCount >= 3) {
        ...
        yield new Event({
          ...
          content: {
            parts: [
              { text: "Multiple errors occurred. Escalating to supervisor." },
            ],
          },
          actions: {
            escalate: true,
...
const debugActions = (session: Session) => {
  ...
  session.events.forEach((event, index) => {
    ...
    console.log(
      `[${index}] ${event.author} at ${new Date(
...
export class CheckoutAgent extends BaseAgent {
  async *runAsyncImpl(context: InvocationContext): AsyncGenerator<Event> {
    ...
    await context.saveArtifact(
      `invoice_${order.id}.json`,
      Buffer.from(invoiceJson),
      "application/json"
    );
    ...
    yield new Event({
      ...
      actions: {
        stateDelta: {
          ...
          "user:lifetimeSpent":
            (context.state["user:lifetimeSpent"] || 0) + order.total,
          ...
          parts: [
            {
              text: `Validation failed: ${validation.errors.join(
                ", "
              )}. Please correct and try again. (Attempt ${attemptCount + 1}/${
                this.MAX_VALIDATION_ATTEMPTS
              })`,
            },
          ],
        },
