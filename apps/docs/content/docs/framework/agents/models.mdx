...
You can pass additional options when creating the OpenRouter instance:
...
Here's a complete example showing how to configure OpenRouter with environment variables:
...
export const envSchema = z.object({
  ...
  OPENROUTER_ONLINE_MODEL: z.string().default("perplexity/sonar-pro"),
});
...
export const getWeatherAgent = () => {
  const weatherAgent = new LlmAgent({
    ...
    tools: [weatherTool],
    instruction: "You are a weather assistant that provides accurate weather information",
  });
