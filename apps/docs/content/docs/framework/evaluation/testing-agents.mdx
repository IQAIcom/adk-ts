...
async function testWorkflowIntegration() {
  ...
  const workflow = [
    ...
    {
      query: "From New York to London on March 15",
      expectedTools: ["book_flight"],
    },
...
The automated evaluation framework is now available as an experimental
feature. APIs may evolve based on user feedback.
...
Start with manual testing using the examples, then gradually build custom test
scripts as your agent functionality grows. This provides immediate validation
while preparing for the upcoming automated framework.
