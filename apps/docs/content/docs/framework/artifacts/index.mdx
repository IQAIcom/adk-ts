...
While session state is perfect for configuration and conversational context,
artifacts are designed for binary data, large files, and content that needs
versioning.
...
const beforeAgentCallback = async (callbackContext: CallbackContext) => {
  try {
    ...
    const version = await callbackContext.saveArtifact(
      "greeting.txt",
      textArtifact,
    );
    ...
    if (loadedArtifact) {
      const text = Buffer.from(
        loadedArtifact.inlineData.data,
        "base64",
      ).toString();
...
Choose appropriate artifact scoping based on data lifecycle requirements -
session for temporary content, user for persistent data.
...
const robustArtifactCallback = async (callbackContext: CallbackContext) => {
  try {
    ...
    return {
      role: "model",
      parts: [
        { text: "Configuration could not be loaded. Using default settings." },
      ],
    };
  }
