...
The `AgentScheduler` runs ADK agents on a recurring schedule using cron expressions or fixed intervals. It handles execution tracking, overlap prevention, graceful shutdown, and real-time event streaming.
