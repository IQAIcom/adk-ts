...
**Creating Hierarchy:** When you create an agent with `subAgents`, ADK automatically establishes the parent-child relationships and sets the `parentAgent` property on each child.
...
// ADK automatically sets: billingAgent.parentAgent === coordinator
...
// Flow: dataCollector saves to state → recommendationEngine reads from state
```
...
**How It Works:** When an agent has sub-agents, ADK automatically enables `AutoFlow`, which provides a `transfer_to_agent()` function. The LLM can call this function to transfer control to an appropriate specialist agent.
...
// "How do I change my password?" → routes to general_support
```
...
// 4. contentEditor → reads draft_content → state["final_content"]
```
...
// 3. contentRefiner → reads both → state["refined_content"]
```
...
class QualityGate extends BaseAgent {
  ...
  protected async *runAsyncImpl(ctx: InvocationContext) {
    const score = parseInt(ctx.session.state.get("quality_score", "0"));
    const iteration = ctx.session.state.get("iteration_count", 0) + 1;
...
// 4. actionExecutor → proceeds based on approval decision
```
