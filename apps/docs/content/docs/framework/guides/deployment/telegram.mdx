...
- **ADK-TS Telegram Bot Project**: Get started with:
  - Use the [ADK-TS CLI](/docs/cli/commands#adk-new) to create a new project: `adk new my-agent --template telegram-bot`
...
- **Bot Not Responding**: Confirm the `TELEGRAM_BOT_TOKEN` is valid by contacting [@BotFather](https://t.me/botfather). Ensure your bot has permission to read messages in group chats if applicable.
- **Rate Limiting**: Telegram has strict rate limits for bots. Check logs for `429 Too Many Requests` errors if your bot is handling high traffic.

## Telegram Best Practices

- **Separation of Concerns**: Keep your bot's logic in the ADK-TS agent and use the Telegram server primarily as a transport layer.
...
<Cards>
  <Card title="🤖 Telegram Bot API" href="https://core.telegram.org/bots/api">
    Official Telegram documentation
  </Card>
  <Card
    title="📚 MCP Telegram Documentation"
