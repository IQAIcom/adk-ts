...
<Callout type="info" title="Workflow vs LLM Agents">
  **Workflow Agents** manage execution flow with predictable patterns
  (sequential → parallel → loop → LangGraph). **LLM Agents** use language models
  for dynamic reasoning and content generation. **Best together:** Use workflow
  agents to orchestrate LLM agents for complex, multi-step processes.
</Callout>
