...
Memory search requires a memory service to be configured in the runner or
agent setup.
...
async function processDocuments(
  ...
  const documents = allArtifacts.filter(
    name =>
  ...
  for (const docName of documents) {
    ...
    switch (params.operation) {
      ...
      case "compare":
        ...
        const related = await toolContext.searchMemory(
...
async function intelligentTransfer(
  ...
  const agentSuggestions = await toolContext.searchMemory(
    `agent for ${params.userQuery}`,
  );
...
async function synthesizeInformation(
  ...
  await toolContext.saveArtifact(
    `synthesis_${params.topic}_${Date.now()}.json`,
    {
...
```typescript
async function robustToolExecution(params: any, toolContext: ToolContext) {
  try {
    ...
    try {
      ...
      console.warn(
        "Memory search failed, using local processing:",
        memoryError,
      );
...
```typescript
async function optimizedToolExecution(params: any, toolContext: ToolContext) {
  // Check cache first
