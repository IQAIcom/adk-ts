...
| Feature             | Railway                                        | Vercel                                 | AWS                                                | Docker (Self-Hosted)                       |
| ------------------- | ---------------------------------------------- | -------------------------------------- | -------------------------------------------------- | ------------------------------------------ |
| **Setup Time**      | 5 minutes (auto-deploy) to 15 minutes (Docker) | 5-10 minutes                           | 15-60 minutes (Lambda faster, ECS/EC2 longer)      | 30+ minutes                                |
| **Docker Required** | No (Nixpacks auto-detect) or optional          | No (auto-detect)                       | Optional (Lambda supports code/containers)         | Yes                                        |
| **Cost**            | $5+/month per service                          | Free tier, then pay-per-use            | Pay-per-use (Lambda) or hourly (ECS/EC2)           | Variable (server cost)                     |
| **Infrastructure**  | Fully managed                                  | Fully managed (serverless)             | Fully managed (managed containers) to self-managed | Self-managed                               |
| **Scaling**         | Automatic                                      | Automatic (zero to thousands)          | Automatic (Lambda/ECS) or manual (EC2)             | Manual configuration                       |
| **CI/CD**           | Built-in GitHub integration                    | Built-in GitHub integration            | Requires setup (CodePipeline, GitHub Actions)      | Requires setup                             |
| **Control**         | Limited (platform constraints)                 | Limited (serverless constraints)       | High (extensive AWS services)                      | Full control                               |
| **Use Case**        | Always-on agents, background tasks             | API routes, webhooks, request-response | Flexible (API to long-running to custom infra)     | Custom needs, complex workloads            |
| **Best For**        | Discord bots, long-running agents              | API agents, serverless workflows       | Enterprise, AWS ecosystem, scalable cloud          | High-scale, cost optimization, flexibility |
| **Execution Time**  | Unlimited                                      | 60-900 seconds (plan dependent)        | 15 min (Lambda), unlimited (ECS/EC2)               | Unlimited                                  |
| **Monitoring**      | Built-in dashboard                             | Built-in analytics and logs            | CloudWatch (comprehensive)                         | Requires setup                             |
...
- Verify Node.js version compatibility (ADK-TS requires Node.js ≥22)
