...
Context management provides agents and tools with access to execution state, services, and session information needed for effective operation. Context objects serve as the bridge between your code and the ADK-TS framework.
...
Think of context as the bridge connecting your agent logic to the ADK-TS
framework - providing access to everything needed for intelligent agent
behavior.
...
ADK-TS provides specialized context objects tailored for different execution scenarios and security requirements.
...
const dynamicInstruction = (ctx: ReadonlyContext): string => {
  ...
  return `You are helping user in session ${
    ctx.invocationId
  }. User context: ${JSON.stringify(userState)}`;
};
```
