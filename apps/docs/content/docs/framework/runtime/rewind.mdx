...
Rewind only affects ADK-TS-managed session state and artifacts. External side effects (API calls, database writes, emails sent) cannot be automatically undone.
