...
- **skip_summarization** (`boolean`): If `true`, returns the agent's response directly without additional LLM processing. If `false` (default), the main agent's LLM processes the response before including it in output

This gives you flexibility in how tool agent responses are handled:

```typescript
// Option 1: Let the main agent process the response
new AgentTool({
  agent: summaryAgent,
  skip_summarization: false, // Main agent will process this
});

// Option 2: Return tool agent's response directly
new AgentTool({
  agent: summaryAgent,
  skip_summarization: true, // Skip extra LLM processing
});
```
...
### Follow Function Design Principles
