...
async function runAgent() {
  ...
  const result = await runner.ask("Hello!");
  console.log(result);
  return result;
}
```
...
<Callout type="info" title="Database peer dependencies">
  Install one of: `better-sqlite3` (SQLite), `pg` (PostgreSQL), or `mysql2`
  (MySQL), depending on your database.
</Callout>
...
<Callout type="warn" title="Choosing an implementation">
  Pick in-memory for local dev, database for most production apps, and cloud for
...
<Cards>
  ...
  <Card
    title="☁️ Vertex AI"
    description="Cloud-managed session service with Google Cloud"
